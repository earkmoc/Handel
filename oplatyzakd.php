...
$z='update `oplatyzak` set ';	//czas start
$z.="`ID_OSOBYUPR`='$ido',";
$z.="`CZAS`=Now(),";
$z.="`MC1`=replace(`MC1`,'-','.'),";
$z.="`MC2`=replace(`MC2`,'-','.') ";
...
$z='select ID, MC1, MC2 from `oplatyzak` ';	//przedostatni zapis (ostatni jest bie¿¹co przetwarzany)
$z.=" order by ID desc limit 1,1";
$w=mysql_query($z);
$w=mysql_fetch_row($w);

$i=$w[2]*1;		//MC2 z przedostatniego
if(100*(round($i-floor($i),2))==12) {$i=$i+0.89;} else {$i=$i+0.01;}		//nastêpny miesi¹c

$z='update `oplatyzak` set ';						//oba musz¹ byæ wiêksze od MC2 z przedostatniego
$z.="`MC1`=".$i.",";
$z.="`MC2`=if(MC2<".$i.",".$i.",MC2) ";
$z.=" where `ID`='$ipole' limit 1";
$w=mysql_query($z);

$z='update `oplatyzak` set ';						// musi byæ MC2 >= MC1
$z.="`MC2`=if(MC2<MC1,MC1,MC2) ";
$z.=" where `ID`='$ipole' limit 1";
$w=mysql_query($z);
...
$z="insert into opldodrob select * from opldod where opldod.TYPOPER='a' order by DATAOPER desc";
$w=mysql_query($z);
//echo $z.'<br>';
...
having (!(ISNULL(przy.DATAOPER) and !ISNULL(odcie.DATAOPER)) 
and (ISNULL(przy.DATAOPER) or ISNULL(odcie.DATAOPER) or przy.DATAOPER>=odcie.DATAOPER))";
$w=mysql_query($z);
//echo $z.'<br>';

$z="Select MC1, MC2 from oplatyzak where ID=$ipole";
$w=mysql_query($z);
$w=mysql_fetch_row($w);
$a=str_replace('-','.',$w[0])*1;
$b=str_replace('-','.',$w[1])*1;
...
$z="insert into opldodrob select * from opldod where opldod.TYPOPER='o' and opldod.DATAOPER<='$d' order by DATAOPER desc";
$w=mysql_query($z);
//echo $z.'<br>';
