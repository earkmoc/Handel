...
$sql="Select DATA1, DATA2 from zestfk1 where ID_OSOBYUPR=osoba_id";
...
if ($ok) {
	...
	$sql.=" sum(BRUTTO0),";
	$sql.=" sum(BRUTTOZW),";
	$sql.=" sum(zestfk1w.WYSWPL),";
	$sql.=" sum(zestfk1w.WYSWPL)-sum(VAT22)-sum(VAT7),";
	$sql.=" sum(VAT22)+sum(VAT7),";
	$sql.=" zestfk1w.ID_OPLATY,";
	$sql.=" abonenci.ZABLOK";
	$sql.=" from zestfk1w ";
	$sql.=" left join abonenci on zestfk1w.IDABONENTA=abonenci.ID left join faktury on (faktury.NRFAKTURY=zestfk1w.NRFAKTURY and faktury.IDABONENTA=zestfk1w.IDABONENTA)";
	$sql.=" group by zestfk1w.NRFAKTURY, zestfk1w.IDABONENTA";
	$sql.=" having abonenci.ZABLOK!='T' and zestfk1w.ID_OPLATY=1";
	$sql.=" order by faktury.DATAWYSTAW, zestfk1w.ID_OPLATY, faktury.NRDOKVAT*1";
...
if ($ok) {
	...
	$nag.='<br>Zestawienie faktur korygujących VAT (punkt Nr 1)';
