...
$sql="Select DATA1, DATA2 from zestfk where ID_OSOBYUPR=osoba_id";
...
if ($ok) {
	...
	$sql.=" sum(BRUTTO0),";
	$sql.=" sum(BRUTTOZW),";
	$sql.=" sum(zestfkw.WYSWPL),";
	$sql.=" sum(zestfkw.WYSWPL)-sum(VAT22)-sum(VAT7),";
	$sql.=" sum(VAT22)+sum(VAT7),";
	$sql.=" zestfkw.ID_OPLATY";
	$sql.=" from zestfkw ";
	$sql.=" left join abonenci on zestfkw.IDABONENTA=abonenci.ID left join faktury on (faktury.NRFAKTURY=zestfkw.NRFAKTURY and faktury.IDABONENTA=zestfkw.IDABONENTA)";
	$sql.=" group by zestfkw.NRFAKTURY, zestfkw.IDABONENTA";
	$sql.=" having zestfkw.ID_OPLATY=osoba_pu";
	$sql.=" order by faktury.DATAWYSTAW, zestfkw.ID_OPLATY, faktury.NRDOKVAT*1";
...
if ($ok) {
	...
	$nag.='<br>Zestawienie faktur korygujących VAT (punkt Nr osoba_pu)';
