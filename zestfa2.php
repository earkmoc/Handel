...
$sql="Select DATA1, DATA2 from zestfa2 where ID_OSOBYUPR=osoba_id";
...
if ($ok) {
	...
	$sql.=" sum(BRUTTO0),";
	$sql.=" sum(BRUTTOZW),";
	$sql.=" sum(zestfa2w.WYSWPL),";
	$sql.=" sum(zestfa2w.WYSWPL)-(sum(BRUTTO22)-round(sum(BRUTTO22)/1.22,2))-(sum(BRUTTO7)-round(sum(BRUTTO7)/1.07,2)),";
	$sql.=" sum(BRUTTO22)-round(sum(BRUTTO22)/1.22,2)+sum(BRUTTO7)-round(sum(BRUTTO7)/1.07,2),";
	$sql.=" zestfa2w.ID_OPLATY,";
	$sql.=" abonenci.ZABLOK";
	$sql.=" from zestfa2w ";
	$sql.=" left join abonenci on zestfa2w.IDABONENTA=abonenci.ID left join faktury on (faktury.NRFAKTURY=zestfa2w.NRFAKTURY and faktury.IDABONENTA=zestfa2w.IDABONENTA)";
	$sql.=" group by zestfa2w.NRFAKTURY, zestfa2w.IDABONENTA";
	$sql.=" having abonenci.ZABLOK='T' and zestfa2w.ID_OPLATY=2";
	$sql.=" order by faktury.DATAWYSTAW, zestfa2w.ID_OPLATY, faktury.NRDOKVAT*1";
...
if ($ok) {
	...
	$nag.='<br>ZESTAWIENIE FAKTUR VAT (punkt Nr 2)';
