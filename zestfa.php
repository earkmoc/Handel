...
$sql="Select DATA1, DATA2 from zestfa where ID_OSOBYUPR=osoba_id";
...
if ($ok) {
	...
	$sql.=" sum(BRUTTO22),";
	$sql.=" round(sum(BRUTTO22)/1.22,2),";
	$sql.=" sum(BRUTTO22)-round(sum(BRUTTO22)/1.22,2),";
	$sql.=" sum(BRUTTO7),";
	$sql.=" round(sum(BRUTTO7)/1.07,2),";
	$sql.=" sum(BRUTTO7)-round(sum(BRUTTO7)/1.07,2),";
	$sql.=" sum(BRUTTO0),";
	$sql.=" sum(BRUTTOZW),";
	$sql.=" sum(zestfaw.WYSWPL),";
	$sql.=" sum(zestfaw.WYSWPL)-(sum(BRUTTO22)-round(sum(BRUTTO22)/1.22,2))-(sum(BRUTTO7)-round(sum(BRUTTO7)/1.07,2)),";
	$sql.=" sum(BRUTTO22)-round(sum(BRUTTO22)/1.22,2)+sum(BRUTTO7)-round(sum(BRUTTO7)/1.07,2),";
	$sql.=" zestfaw.ID_OPLATY";
	$sql.=" from zestfaw ";
	$sql.=" left join abonenci on zestfaw.IDABONENTA=abonenci.ID left join faktury on (faktury.NRFAKTURY=zestfaw.NRFAKTURY and faktury.IDABONENTA=zestfaw.IDABONENTA)";
	$sql.=" group by zestfaw.NRFAKTURY, zestfaw.IDABONENTA";
	$sql.=" having 1=1";		//zestfaw.ID_OPLATY=osoba_pu";
	$sql.=" order by faktury.DATAWYSTAW, zestfaw.ID_OPLATY, faktury.NRDOKVAT*1";
...
if ($ok) {
	...
	$nag.='<br>ZESTAWIENIE FAKTUR VAT';	// (punkt Nr osoba_pu)';
...
//	echo $nag=str_replace('-','÷',$nag);
	...
	while ($r=mysql_fetch_row($w)) {
		...
		$line.=Chr(144);
		$line.='<br><br><br><br>.';
//		$line=str_replace('"12CPI"',Chr(162),$line);		//Chr(27).'M',$w);					//1
//		$line=str_replace('"15CPI"',Chr(164),$line);		//Chr(27).'g',$w);					//1
//		$line=str_replace('"17CPI"',Chr(136),$line);		//Chr(15),$w);							//1
//		$line=str_replace('"W0"',Chr(129),$line);			//Chr(27).'W0',$w);					//1
//		$line=str_replace('"W1"',Chr(131),$line);			//Chr(27).'W1',$w);					//1
