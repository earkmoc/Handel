...
$z="update opldodg set IDABONENTA=$ida, TYPOPER='u', ID_OSOBYUPR=$ido, CZAS=Now() where ID=$ipole limit 1";
$w=mysql_query($z);
...
$z="Select ID from tabele where NAZWA='typyumowud'";		//typ wybrany z tabeli
$w=mysql_query($z); $w=mysql_fetch_row($w); $w=$w[0];
...
if ($w['WYS1WPL']>0) {
	if ($w['TERM1WPL']==0) {							//natychmiast
		$da=$dt;
	}
	else {
		$md++;												//przysz³y miesi¹c: 4 -> 5
		if ($md>12) {$md=1; $rd++;}					//12.2006 -> 1.2007
		$dd=$w['TERM1WPL'];								//któregoœ dnia
		$da=sprintf("%'04d",$rd);
		$da.='-'.sprintf("%'02d",$md);
		$da.='-'.sprintf("%'02d",$dd);
	}
}
//$da=substr($dt,0,8).sprintf("%'02d",$w['TERM1WPL']);}		// '2006-04-11'  ->   '2006-04-'
$di++;
...
if ($w['WYS2WPL']>0) {
	$md++;							// nastêpny miesi¹c, bo druga rata
	if ($md>12) {$md=1; $rd++;}
...
for($i=1;$i<=$w['ILOSCRAT'];$i++) {
	$md++;							// nastêpny miesi¹c, bo kolejna rata
	if ($md>12) {$md=1; $rd++;}
