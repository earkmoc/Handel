...
$sql="Select DATA1, DATA2 from zestfa1 where ID_OSOBYUPR=osoba_id";
...
if ($ok) {
	...
	$sql.=" sum(BRUTTO0),";
	$sql.=" sum(BRUTTOZW),";
	$sql.=" sum(zestfa1w.WYSWPL),";
	$sql.=" sum(zestfa1w.WYSWPL)-(sum(BRUTTO22)-round(sum(BRUTTO22)/1.22,2))-(sum(BRUTTO7)-round(sum(BRUTTO7)/1.07,2)),";
	$sql.=" sum(BRUTTO22)-round(sum(BRUTTO22)/1.22,2)+sum(BRUTTO7)-round(sum(BRUTTO7)/1.07,2),";
	$sql.=" zestfa1w.ID_OPLATY,";
	$sql.=" abonenci.ZABLOK";
	$sql.=" from zestfa1w ";
	$sql.=" left join abonenci on zestfa1w.IDABONENTA=abonenci.ID left join faktury on (faktury.NRFAKTURY=zestfa1w.NRFAKTURY and faktury.IDABONENTA=zestfa1w.IDABONENTA)";
	$sql.=" group by zestfa1w.NRFAKTURY, zestfa1w.IDABONENTA";
	$sql.=" having abonenci.ZABLOK!='T' and zestfa1w.ID_OPLATY=1";
	$sql.=" order by faktury.DATAWYSTAW, zestfa1w.ID_OPLATY, faktury.NRDOKVAT*1";
...
if ($ok) {
	...
	$nag.='<br>ZESTAWIENIE FAKTUR VAT (punkt Nr 1)';
