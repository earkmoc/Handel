...
$z="Select TYP, Year(DATAW), Year(DATAO), Year(CurDate()), TOWCENNIK from dokum where ID=$idd";
$w=mysql_query($z); $w=mysql_fetch_row($w);
$doktyp=$w[0];
...
$cenik=$w[4];  //cennik jest ujemny jeœli ceny maj¹ byæ zamro¿one
...
//mo¿na wy³¹czyæ ograniczniki netnet
if ($cenik<=0) {$mpodm=$cenik;}         
if ($mpodm_pre) {$mpodm=$mpodm_pre;}

$korekta=false;
if (substr($doktyp,-1,1)=='K') {$korekta=true;$mpodm=0;}
...
//	$idd=$r[4];
	...
	$rabat=(($status=='U')?0:$rabat);
  	...
  	$cena=($korekta&&($cenaOryg<>0)?$cenaOryg:myRound($cena*(100-$rabat)*0.01));
...
if (!$all_spec
   ...
   &&$dostawa_posztukwpaczce
   &&$dostawa_cenazapaczke
   &&($dostawa_ilepaczek<>1.1)
   &&($dostawa_posztukwpaczce<>1.2)
   &&($dostawa_cenazapaczke<>1.3)) {
	
	$cena2=0;
	$ilosc2=0;
	
	$dostawa_wartosc=myRound($dostawa_ilepaczek*$dostawa_cenazapaczke);
	$cena=myRound($towary_wpaczce*$dostawa_cenazapaczke/$dostawa_posztukwpaczce);
	$ilosc=myRound($dostawa_ilepaczek*$dostawa_posztukwpaczce/$towary_wpaczce,0);
	$wartosc=myRound($cena*$ilosc);

	if ($wartosc>$dostawa_wartosc) {
		$cena2=$cena-0.01;
		$ilosc2=0;
		do {
			$ilosc2++;
			$ilosc1=$ilosc-$ilosc2;
			$wartosc1=myRound($cena *$ilosc1);
			$wartosc2=myRound($cena2*$ilosc2);
			$wartosc3=myRound($wartosc1+$wartosc2);
		} while (($ilosc2<=$ilosc)&&($wartosc3<>$dostawa_wartosc));
	}
	...
	if ($ilosc2<>0) {
		...
		$z=("insert
			   into spec 
			 select 0, ID_D, ID_T, $cena2, $ilosc2, 0, $cena2, NETTO, KWOTAVAT, BRUTTO, CENABRUTTO, STAWKAVAT
			   from spec
			  where ID=$ipole limit 1
		");
		mysql_query($z);
		$ipole2=mysql_insert_id();			// identyfikator nowego wiersza w tabeli
	}
}
...
if ($idd) {
   ...
   if ($odcen=='Brutto') {
		...
		$z="select sum(BRUTTO) from spec where ID_D=$idd and STAWKAVAT*1=23";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$b23=($w[0]?$w[0]:0);
		$n23=myRound($b23*100/123);
		$v23=$b23-$n23;
		
		$z="select sum(BRUTTO) from spec where ID_D=$idd and STAWKAVAT*1=22";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$b22=($w[0]?$w[0]:0);
		$n22=myRound($b22*100/122);
		$v22=$b22-$n22;
		
		$z="select sum(BRUTTO) from spec where ID_D=$idd and STAWKAVAT*1=8";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$b8=($w[0]?$w[0]:0);
		$n8=myRound($b8*100/108);
		$v8=$b8-$n8;
		
		$z="select sum(BRUTTO) from spec where ID_D=$idd and STAWKAVAT*1=7";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$b7=($w[0]?$w[0]:0);
		$n7=myRound($b7*100/107);
		$v7=$b7-$n7;
		
		$z="select sum(BRUTTO) from spec where ID_D=$idd and STAWKAVAT*1=5";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$b5=($w[0]?$w[0]:0);
		$n5=myRound($b5*100/105);
		$v5=$b5-$n5;
		
		$z="select sum(BRUTTO) from spec where ID_D=$idd and (STAWKAVAT='0%' or STAWKAVAT=' 0%')";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$b0=($w[0]?$w[0]:0);
		$n0=$b0;
		...
		$z="select sum(NETTO) from spec where ID_D=$idd and STAWKAVAT*1=23";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$n23=($w[0]?$w[0]:0);
		$v23=myRound($n23*0.23);
		
		$z="select sum(NETTO) from spec where ID_D=$idd and STAWKAVAT*1=22";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$n22=($w[0]?$w[0]:0);
		$v22=myRound($n22*0.22);
		
		$z="select sum(NETTO) from spec where ID_D=$idd and STAWKAVAT*1=8";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$n8=($w[0]?$w[0]:0);
		$v8=myRound($n8*0.08);
		
		$z="select sum(NETTO) from spec where ID_D=$idd and STAWKAVAT*1=7";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$n7=($w[0]?$w[0]:0);
		$v7=myRound($n7*0.07);
		
		$z="select sum(NETTO) from spec where ID_D=$idd and STAWKAVAT*1=5";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$n5=($w[0]?$w[0]:0);
		$v5=myRound($n5*0.05);
		
		$z="select sum(NETTO) from spec where ID_D=$idd and (STAWKAVAT='0%' or STAWKAVAT=' 0%')";
		$w=mysql_query($z); $w=mysql_fetch_row($w);
		$n0=($w[0]?$w[0]:0);
