...
if ($blokada=='O') {	//otwarty, wiêc ZAMYKAMY
	...
	if ($mn==1) {$z="insert into spec select 0, -spe.ID_D, spe.ID_T, spe.CENA, spe.ILOSC, 0, 0 from spec as spe where spe.ID_D=$ipole and spe.ILOSC<>0";$w=mysql_query($z);}
	else {
		$z="insert into spec select 0, -spe.ID_D, spe.ID_T, magazyny.CENA_Z, magazyny.ILOSC, 0, 0 from spec as spe left join magazyny on (magazyny.ID_X=$ma and magazyny.ID_T=spe.ID_T) where spe.ID_D=$ipole and spe.ILOSC>=0 order by magazyny.DATA_Z, magazyny.ILOSC";$w=mysql_query($z);
...
//numer
   ...
   if ((substr($nrdok,-4,4)=='AUTO')||(substr($nrdok,-7,7)=='AUTOMAT')) {
		...
		$z="update dokum set BLOKADA='', CZAS=CurTime(), INDEKS='$nrdok$maska' where ID=$ipole limit 1";$w=mysql_query($z);
	}
	else {
		$z="update dokum set BLOKADA='', CZAS=CurTime() where ID=$ipole limit 1";$w=mysql_query($z);	//finito
	}

//	$komunikat='Dokument zamkniêty';
}
elseif ($blokada=='') {	//zamkniêty, wiêc OTWIERAMY
...
//nale¿noœci i korekty u kontrahenta
	...
	$zz="select sum(WARTOSC-WPLACONO) from dokum where NABYWCA=$nab";
	...
	$zz="update firmy SET NALEZNOSCI=$naleznosci, KOREKTY=-$ww where ID=$nab limit 1";
	$ww=mysql_query($zz);
?>
