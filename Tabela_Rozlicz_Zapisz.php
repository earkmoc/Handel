...
if (substr($w[2],0,1)=='0') {	//data zerowa
		...
		elseif ($dane['KPLUBBANK']=='.') {		//UMorzenie
			$dane['NUMERUM']='.';
		}
		...
		if ($dane['KWOTA']<>0) {
			...
			if ($dane['GENE_FV']==='T') {		//generujemy fakturê
					...
					$z.="','".sprintf("%'02d",substr($dane['DATAWPLATY'],5,2));		// m-c
				...
				if ($zaznaczone) {
					...
					for($i=0;$i<count($x);$i++) {
						...
						if ($o['KWOTA']<>0) {
							...
							$z.=$osoba_id;
							$z.="','";
							if ($dane['NUMERFV']&&!$o['NRFAKTURY']&&(substr($o['DODNIA'],0,7)==date('Y-m'))&&(trim($o[14])!='BRAK')) {
								$z.=$dane['NUMERFV'];
								$sumaFV+=$o['KWOTA'];
							}
							else {
			...
			if ($dane['KWPLA']<>0) {						// wp³ata gotówk¹
				...
				if ($zaznaczone) {
					...
					elseif ($dane['NUMERUM']=='.') {				//UM prawie jak KP
						$z="Select * from typydok where LITERA='.' limit 1";
				...
				$www=1;
				if ($dane['KPLUBBANK']>='1') {	// bank
					$z="Select ID, KWOTA from dokwplat where ";		// sprawdŸ dubel w "dokwplat"
					$z.="KPLUBBANK='".$dane['KPLUBBANK'];
					$z.="' and ";
					$z.="NRDOKUM='".$dane['NUMERKP'].$dane['MASKAKP'];
					$z.="' and ";
					$z.="DATAPRZYJ='".$dane['DATAPRZYJ'];
					$z.="' and ";
					$z.="IDOPERATOR='".$dane['IDOPERATOR'];
					$z.="' and ";
					$z.="IDABONENTA='".$dane['IDABONENTA'];
					$z.="'";
					if ($w=mysql_query($z)) {
						$w=mysql_fetch_row($w);
					}
				...
				if ($www==0 || $w[0]==0) {
					...
					$z="update dokwplat SET ";		// zapis danych do "dokwplat"
					$z.="KWOTA=".($w[1]+$dane['KWPLA']);
					$z.=" where ID=".$w[0];
					$w=mysql_query($z);	// zapis danych do "dokwplat"
					$sql.=$z; $sql.="<br>";

					$dane['ID_DOKWPLAT']=$w[0];
					$dane['KW_DOKWPLAT']=$dane['KWPLA'];
				}		//if ($w[0]==0)
...
$ww=mysql_query($zz);
        ...
        if (false) {
                ...
                elseif ($dane['DRUK_KP']==='T') {
						if (!$dane['NUMERKP']&&$dane['NUMERRN']) {
							echo 'WydrukWzor.php?natab=abonenci&wzor=RN&ipole='.$dane['ID_DOKWPLAT'];
						}
						elseif ($dane['NUMERUM']) {
							echo 'WydrukWzor.php?natab=abonenci&wzor=UM&ipole='.$dane['ID_DOKWPLAT'];
						}
						else {
							echo 'WydrukWzor.php?natab=abonenci&wzor=KP&ipole='.$dane['ID_DOKWPLAT'];
						}
					 }
					 else {echo 'Tabela.php?tabela='.$tabelaa;};
        }
        echo "'";
        echo '">';
//      echo $sql;
