...
session_start();		//inicjacja formularza op³aty jednorazowej
...
$z="Select ID from tabele where NAZWA='typyumowoj'";		//parametr typu wybrany z tabeli
$w=mysql_query($z); $w=mysql_fetch_row($w); $w=$w[0];

$z="Select ID_POZYCJI from tabeles where (ID_TABELE=$w or ID_TABELE=-$w) and ID_OSOBY=$ido";	//jest ID typu op³.
$w=mysql_query($z); $w=mysql_fetch_row($w); $w=$w[0];								//ostatnio u¿ytej

$z="Select concat('1',TYPUMOWY), CENA from typyumow where ID=$w";				//jest reszta
