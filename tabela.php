...
if ($_GET['tabSubNazwa']) {$_SESSION['tabSubNazwa']=$_GET['tabSubNazwa'];}
$tabSubNazwa=$_SESSION['tabSubNazwa'];

if ($_GET['nieprzepisuj']) {$_SESSION['nieprzepisuj']=1;}
if ($_GET['przepisuj'])    {$_SESSION['nieprzepisuj']='';}

$ox=0;
$oy=0;
//require('skladuj_zmienne.php');
if ( (($_GET['tabela']=='tabele')&&($ido<>1))
//||(($_GET['tabela']=='tabelez')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='wzoryumow')&&($_SESSION['osoba_dos']<>'T'))
||(((strtoupper(substr($_GET['tabela'],0,9)))=='DOKUMENTB')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='todo')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='tab_obl')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='magazyny')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='osobyz')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='towaryold')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='towarydubl')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='towarySTAN')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='towarySTPZ')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='towarySPZK')&&($_SESSION['osoba_dos']<>'T'))
||(((strtoupper(substr($_GET['tabela'],0,9)))=='PARAMETRY')&&($_SESSION['osoba_dos']<>'T'))
||(($_GET['tabela']=='doktypy')&&($_SESSION['osoba_dos']<>'T'))
) {
	?>
<html>
<head>
...
//echo '<link type="text/css" href="css/start/jquery-ui-1.8.13.custom.css" rel="stylesheet" />';
//include('jquery.php');
}
...
if ($_POST['idtab']) {
	...
	if (($ntab_master)==='dokumentBA') {$_SESSION['ntab_mast']=$ntab_master;};
...
if ($w){
        ...
        $sql=str_replace('$tabSubNazwa',$tabSubNazwa,$sql);
        if (!$sql) { exit;}
        else {
                $w=explode("\n",$sql);
                $z=trim($w[0]);
...
//        $tyta=StripSlashes($wa['OPIS']);
        ...
        $sqla=str_replace('$tabSubNazwa',$tabSubNazwa,$sqla);

        $funa=StripSlashes($wa['FUNKCJE']);
        $funa=str_replace('$tabSubNazwa',$tabSubNazwa,$funa);
  ...
  if ($zprequery) {
	for($k=0;$k<count($prequery);$k++) {
		...
		if (substr($prequery[$k],0,1)=='?') {
			...
			$wa=mysql_query($prequery[$k]);
			if (strtoupper(substr(trim($prequery[$k]),0,6))=='SELECT') {	//je&#181;li typu "SELECT"
				$qs=mysql_fetch_row($wa);										//to co&#181; zwraca
				$i=$k+1;					//nastêpne prequery
				if ($i<count($prequery)) {			//je&#181;li s¹ nastêpne, to
...
if (!$w) {
	...
	exit;}
        ...
        $fun=str_replace('$tabSubNazwa',$tabSubNazwa,$fun);
...
//			               $w[$i]=str_replace('CENA_Z','CENA_Z*0',trim($w[$i]));
                        ...
                        elseif (substr($w[$i],0,6)=='having') {$zhaving=' '.$w[$i];}
                           ...
                           if(!$l[1]) {
               					...
               					$tn[$i-1]=trim($l[1]);
               					if (count(explode("[",$tn[$i-1]))>1) {	//s¹ jakie&#181; odwo³ania w nazwie kolumny
  ...
  if ($zprequery) {
	for($k=0;$k<count($prequery);$k++) {
		...
		if (substr($prequery[$k],0,1)=='?') {
			...
			$w=mysql_query($prequery[$k]);
			if (strtoupper(substr(trim($prequery[$k]),0,6))=='SELECT') {	//je&#181;li typu "SELECT"
				$qs=mysql_fetch_row($w);										//to co&#181; zwraca
				$i=$k+1;					//nastêpne prequery
				if ($i<count($prequery)) {			//je&#181;li s¹ nastêpne, to
					for ($j=0;$j<count($qs);$j++) {		//korzystaj¹ ze swoich wyników
						$prequery[$i]=str_replace('{'.$j.'}',$qs[$j],$prequery[$i]);
...
if (strpos($z,'where')) {
   ...
   if (($baza=='towary')&&(strpos($z,'towary.INDEKS  , towary.NAZWA'))) {
      $z=str_replace('from towary left','from towary force index (INDEKS_2) left',$z);
   }
   if (($baza=='towary')&&(strpos($z,'towary.DOSTAWCA  , towary.NAZWA'))) {
      $z=str_replace('from towary left','from towary force index (DOSTAWCA) left',$z);
   }
   if (($baza=='towary')&&(strpos($z,'order by towary.NAZWA'))) {
      $z=str_replace('from towary left','from towary force index (NAZWA) left',$z);
   }
} else {
   if (($baza=='firmy')&&($_SESSION['osoba_dos']<>'T')) {
...
if ($strr<$str) {
	...
	$sql.='<br>2. '.$z.';';
	$w=mysql_query($z);				// czê&#181;æ zawarto&#181;ci tabeli Slave
	if ($w) {$n=mysql_num_rows($w);} else {$n=0;};
}

if (!$n) {					//jak z limitem, to puchy
	$sql.='<br>zz. '.$zz.';';
	$w=mysql_query($zz);			// ca³a zawarto&#181;æ tabeli Slave bez limitu
	if ($w) {$n=mysql_num_rows($w);} else {$n=0;};
	if (!$n) {						// total puchy
		$r=1;
		$str=1;
		echo '$str=1;';
		echo "\n";
	} else {							//jak bez limitu, to co&#181; jest
		$sql.=';<br>n='.$n.';';
		$sql.=';<br>rrr='.$rrr.';';
...
function Start(){
	...
	$rr=tab.summary;            //przejêcie praktycznej ilo¶ci wierszy na stronie z tajnego elementu tabeli wype³nianego przez PHP podczas definowania strony
...
// w prawo
        if ((event.keyCode==39)&&$c<$cc) {
            $c+=1;
            $zombie=false;
        }

// w lewo
        if ((event.keyCode==37)&&$c>1) {
            $c-=1;
            $zombie=false;
        }
	...
	if (!$fun) { exit;}
		...
		for($i=0;$i<$cc;$i++) {
			...
			if ($l[0]=='Enter') {
				$ok=true;
				if ($l[3]) {
				  if (($l[3]=='2')||($l[3]=='3')&&($_SESSION['osoba_dos']<>'T'))
				  {
					$ok=false;
				  }
				  else
				  {
					if (count(explode("[",$l[3]))>1) {	//s¹ jakie&#181; odwo³ania do mastera
						$zz=explode('.',$tabelap);	//[1].[2]
						for($ii=0;$ii<count($zz);$ii++) {
							$jj=substr($zz[$ii],1)*1;
        ...
        if ($fun) {
			...
			$x=$l[0];
			   ...
			   if ($l[1]<>'') {
				$ok=true;
				if ($l[3]) {
				  if ((substr($l[3],0,1)=='2')||(substr($l[3],0,1)=='3')&&($_SESSION['osoba_dos']<>'T'))
				  {
					$ok=false;
				  }
				  else
...
function Zaznaczal(){
   if (f0.zaznaczone.value=='x') {		//co&#181; zaznacza³, ale kolejno&#181;æ trudno ustaliæ
        f0.zaznaczone.value='';
        for($i=0;$i<$rr;$i++) {
...
function WieleStron($h) {
	if ($h=prompt('Podaj ilo¶æ wierszy na stronie tej tabeli ( jednorazowa zmiana tymczasowa ) :',$rr)) {
		...
		$str=($str-1)/$h+1;	//strona z t¹ pozycj¹ w nowych warunkach ilo&#181;ci wierszy na stronie
                f0.strpole.value=$str;
	        f0.opole.value="S";
		f0.odswiez.click();
	}
...
if ($fun) {
	...
	for($i=0;$i<$cc;$i++) {
		$l=explode("|",$f[$i]);
		if (($l[1])&&(($l[3]=='1')||(($l[3]=='2')&&($_SESSION['osoba_dos']=='T')))) {
			$ileobok++;
		} elseif ($l[1]) {
			$ok=true;
			if ($l[3]) {
		...
		for($i=0;$i<1;$i++){
			...
			for($j=0;$j<$mca;$j++){
				...
				if (count($z=explode(":",$szera[$j]))>1) {                        //obrazek
					...
					else {echo substr($tra[$j],0,$buf*1);};
				}
				echo '</td>';
				echo "\n";
			}
...
if ($mr==0) {                // pusta sub tabela
	...
	if ($sumyok) {                // wiersz sum
		...
		for($j=0;$j<$mc;$j++) {
			...
			//echo $szer[$j];
			if (!$szer[$j]) {
				echo $sumy[$j];	// kolumny bez okre&#181;lonej szeroko&#181;ci
			} elseif ($szer[$j]==='+') {
